...
def filter_fasta_by_hmm(
    ...
    if output_fasta is None:
        output_fasta = set_default_output_path(input_fasta, tag=f"filtered_{hmm_model.stem}")
    else:
        output_fasta = Path(output_fasta)
...
def convert_phylip_to_fasta_aln(input_phylip: Path, output_file: Path = None) -> None:
    ...
    """
    input_phylip = Path(input_phylip)
...
def convert_stockholm_to_fasta_aln(
    ...
    """
    input_stockholm = Path(input_stockholm)
    if output_fasta is None:
...
def split_reference_from_query_alignments(
    ref_query_msa: Path, ref_ids: set = None, ref_prefix: str = None, output_dir: Path = None
) -> None:
    """
    Separate reference sequences from query sequences in msa fasta file
    """
    ref_query_msa = Path(ref_query_msa)
