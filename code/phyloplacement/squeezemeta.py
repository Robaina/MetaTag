...
class SqueezeMetaOutputParser():
    def __init__(self, squeeze_output: Path) -> None: 
        ...
        self._squeeze_out = pd.read_csv(
            squeeze_output,
            sep="\t", header=1) #.dropna(subset=["Tax"])
    ...
    def replaceByPlacementTaxopath(self, placement_assignments: Path,
        ...
        placed_df.taxopath = placed_df.taxopath.apply(lambda p: "" if p == "Unspecified" else p)
        filtered_squeeze = self._squeeze_out[self._squeeze_out["ORF ID"].isin(placed_df["query_name"])].copy()
        for i, row in filtered_squeeze.iterrows():
            gtdb_tax = placed_df.loc[placed_df.query_name == row["ORF ID"], "taxopath"].item()
...
class SqueezeMetaTaxonomyParser():
    def __init__(self, squeeze_output: Path) -> None: 
        ...
        self._squeeze_out = pd.read_csv(
            squeeze_output,
            sep="\t", header=None,
            skiprows=1).rename({0: "ORF ID", 1: "Tax"}, axis=1)
        with open(squeeze_output) as f:
            firstline = f.readline()
        self._metaheader = firstline
