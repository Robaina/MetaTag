...
setwd("D:/OneDrive/TRABAJO.CSIC/prj.traits/campaña_Envision/analysis/n_cycle")                     #<--- CHANGE ACCORDINGLY !!!

#' Please give the file name of the table containing the variables for analysis
input_file <-"master_input_correlations.tsv"              #<--- CHANGE ACCORDINGLY !!!
...
otu_variables_start <- 26                                        #<--- CHANGE ACCORDINGLY !!!
...
my_rcorr <- rcorr(as.matrix(my_scaled_data), type = "spearman")
...
for (i in 1:dim(corr_pval_cutoff)[1]) {
  ...
  corr_text <- paste("Spearman's r:", round(as.numeric(corr_pval_cutoff[i, 3]), 4), sep = "")
